...
def parse_chromosome_info(chrom_name):
    ...
    if len(parts) >= 3:
        ...
        sample = "_".join(
            parts[:-2]
        )  # Handle sample names that might contain underscores
        return sample, hap, chrom
    else:
        raise ValueError(
            "Your contigs aren't named according to the PanSN naming spec, or --pansn-chr is wrong"
        )
            ...
            for line in file:
                ...
                for gid in gids:
                    if gid:
                        gid2oid[gid] = oid
    print(
        f"Loaded mapping of {len(gid2oid)} genes to {len(set(gid2oid.values()))} orthogroups",
        file=stderr,
    )
    return gid2oid


def extract_gene_order_from_gff(
    gff_file, orthogroup_map, feature_type="gene", pansn_chr="_"
):
    ...
    for chromosome in chromosome_genes:
        chromosome_genes[chromosome].sort(
    ...
    print(
        f"Processed {total_genes} genes, {missing_genes} genes not found in orthogroup mapping",
        file=stderr,
    )
...
def filter_contained_segments_with_positions(colinear_segments):
    ...
    for i, segment in enumerate(sorted_segments):
        ...
        for j in range(i):
            ...
            larger_segment_paths = set(
                colinear_segments[larger_segment]["path_indices"]
            )

            # If segment is a substring of larger_segment and has the same path occurrences
            if _is_subpath(segment, larger_segment) and segment_paths.issubset(
                larger_segment_paths
            ):
                is_contained = True
                break
...
def detect_colinear_segments_optimized(
    paths, gene_positions_list, min_shared_paths=2, min_length=3
):
    """
        ...
        if len(positions_by_path) >= min_shared_paths:
            ...
            for path_idx in positions_by_path:
                if len(positions_by_path[path_idx]) > max_positions_per_path:
                    ...
                    positions_by_path[path_idx] = positions_by_path[path_idx][
                        :max_positions_per_path
                    ]
            ...
            if len(valid_path_idxs) >= min_shared_paths:
                ...
                for path_idxs in combinations(valid_path_idxs, min_shared_paths):
                    ...
                    pos_combinations = _get_position_combinations(
                        positions_by_path, path_idxs
                    )
                    ...
                    for pos_combination in pos_combinations:
                        ...
                        if is_repeating:
                            max_segment = _find_max_repeating_segment(
                                tuple_paths, path_idxs, pos_combination, min_segment[0]
                            )
                        else:
                            max_segment = _find_max_shared_segment(
                                tuple_paths, path_idxs, pos_combination, min_length
                            )
                        ...
                        if max_segment and len(max_segment) >= min_length:
                            ...
                            for idx, path_idx in enumerate(path_idxs):
                                ...
                                pos_info = {
                                    ...
                                    "start_idx": start_idx,
                                    "end_idx": end_idx,
                                    "start_pos": gene_positions_list[path_idx][
                                        start_idx
                                    ]["start"],
                                    "end_pos": gene_positions_list[path_idx][end_idx][
                                        "end"
                                    ],
                                    "chrom": gene_positions_list[path_idx][end_idx][
                                        "chrom"
                                    ],
                                }
                                ...
                                for existing_pos in colinear_segments[max_segment][
                                    "positions"
                                ]:
                                    if (
                                        existing_pos["path_idx"] == path_idx
                                        and existing_pos["start_idx"] == start_idx
                                    ):
                                        already_recorded = True
                                        break

                                if not already_recorded:
                                    colinear_segments[max_segment]["positions"].append(
                                        pos_info
                                    )
...
def _get_position_combinations(positions_by_path, path_idxs):
    ...
    if total_combinations > max_safe_combinations:
        reduction_factor = int(
            ceil(
                pow(total_combinations / max_safe_combinations, 1 / len(position_lists))
            )
        )
        position_lists = [
            lst[: max(1, len(lst) // reduction_factor)] for lst in position_lists
        ]
...
def _find_max_repeating_segment(paths, path_idxs, positions, repeating_element):
    ...
    for path_idx, pos in enumerate(positions):
        ...
        while (
            pos + count < path_lengths[path_idx]
            and path[pos + count] == repeating_element
        ):
            count += 1
...
def _find_max_shared_segment(paths, path_idxs, positions, min_length):
    ...
    max_possible_length = min(
        length - pos for length, pos in zip(path_lengths, positions)
    )
    ...
    for length in range(min_length, max_possible_length + 1):
        current_segments = [
            path[pos : pos + length] for path, pos in zip(relevant_paths, positions)
        ]
...
def identify_neighbourhoods(
    colinear_segments, sample_info, gene_positions_list, just_chrom
):
    """
    ...
    for left_segment, right_segment in segment_pairs:
        ...
        for sample_idx, segments in segments_by_sample.items():
            for i in range(len(segments) - 1):
                if (
                    segments[i]["segment"] == left_segment
                    and segments[i + 1]["segment"] == right_segment
                ):
                    ...
                    region = {
                        ...
                        "start_pos": segments[i][
                            "end_pos"
                        ],  # End of left syntenic block
                        "end_pos": segments[i + 1][
                            "start_pos"
                        ],  # Start of right syntenic block
...
def orthograf(
    ...
    for gff_path in gff_files:
        if not os.path.exists(gff_path):
            print(
                f"Warning: GFF file '{gff_path}' does not exist. Skipping.", file=stderr
            )
            continue

        chromosome_paths = extract_gene_order_from_gff(
            gff_path, orthogroup_map, pansn_chr=pansn_chr
        )
    ...
    for chrom, paths_info in chromosome_groups.items():
        ...
        print(
            f"Analyzing chromosome {chrom} across {len(paths_info)} samples/haplotypes",
            file=stderr,
        )
        ...
        neighbourhoods = identify_neighbourhoods(
            colinear_segments, sample_info, gene_positions_list, just_chrom=chrom
        )
            ...
            "colinear_segments": [
                {
                    ...
                    "samples": [
                        sample_info[idx] for idx in segment_info["path_indices"]
                    ],
                    "positions": [
                        {
        ...
        # Print summary
        print(
            f"Found {len(colinear_segments)} co-linear segments shared by at least {min_shared_paths} samples",
            file=stderr,
        )
        print(
            f"Found {len(neighbourhoods)} neighbourhoods between syntenic blocks",
            file=stderr,
        )
        print("\n")
    ...
    if bedfile:
        for chrom, result in results.items():
            for i, seg in enumerate(
                sorted(
                    result["colinear_segments"],
                    key=lambda x: mean(y["start_pos"] for y in x["positions"]),
                )
            ):
                print(
                    f"\n# Segment of {seg['length']} orthogroups shared by {seg['shared_by']} samples",
                    file=bedfile,
                )
    ...
    if neighbourhood_file:
        for chrom, result in results.items():
            for nh in result["neighbourhoods"]:
                print(
                    f"\n# Neighbourhood {nh['id']} between syntenic blocks",
                    file=neighbourhood_file,
                )
                print(
                    "# Left block:",
                    " -> ".join(nh["left_segment"]),
                    file=neighbourhood_file,
                )
                print(
                    "# Right block:",
                    " -> ".join(nh["right_segment"]),
                    file=neighbourhood_file,
                )
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Analyze orthogroup paths from GFF files"
    )
    parser.add_argument("gff_files", nargs="+", help="GFF files to process")
    parser.add_argument(
        "--ogs", help="GeneID-OGID mapping file from either diamond or orthofinder"
    )
    parser.add_argument(
        "--og-format",
        help="Which format is --ogs?",
        choices=("diamond", "orthofinder"),
        default="diamond",
    )
    parser.add_argument(
        "--min-shared",
        type=int,
        default=17,
        help="Minimum number of samples sharing a segment",
    )
    parser.add_argument(
        "--min-length",
        type=int,
        default=5,
        help="Minimum length of co-linear segments in genes",
    )
    parser.add_argument(
        "--out-syntbed",
        help="Bedfile output for syntenic regions",
        type=argparse.FileType("wt"),
    )
    parser.add_argument(
        "--out-nhbed",
        help="Bedfile output for neighbourhood regions",
        type=argparse.FileType("wt"),
    )
    parser.add_argument(
        "--out-json", help="JSON data dump output", type=argparse.FileType("wt")
    )
    parser.add_argument(
        "--pansn-chr",
        default="_",
        help="Separator between sample, hap, and chrom in PanSN names (see PanSN spec, default is _ NOT #)",
    )
    ...
    res = orthograf(
        ...
        args.min_shared,
        args.min_length,
        bedfile=args.out_syntbed,
        neighbourhood_file=args.out_nhbed,
